...
class Lang:  # ont-hot vector 인코딩 다른 embedding 필요 language
    ...
    def addWord(self, word):
        if word not in self.word2index.keys():  # initializer
...
class GloveSaver():
    def __init__(self, glove_path):
        ...
        self.vectors = bcolz.carray(np.zeros(1), rootdir="%s/6b.50.dat"%(glove_path), mode='w')
        with open("%s/glove.6B.50d.txt"%(glove_path), 'rb') as f:
            for l in f:
                line = l.decode().split()
                word = line[0]
            ...
            self.vectors = bcolz.carray(self.vectors[1:].reshape((400000,50)), rootdir="%s/6b.50.dat"%(glove_path), mode='w')
            self.vectors.flush()
            pickle.dump(self.words, open("%s/6B.50_words.pkl"%(glove_path), 'wb'))
            pickle.dump(self.word2idx, open("%s/6B.50_idx.pkl"%(glove_path), 'wb'))
...
class GloveEmbedder():
    def __init__(self, glove_path):
        self.vectors = bcolz.open("%s/6b.50.dat"%(glove_path))[:]
        self.words = pickle.load(open("%s/6B.50_words.pkl"%(glove_path), 'rb'))
        self.word2idx = pickle.load(open("%s/6B.50_idx.pkl"%(glove_path), 'rb'))
